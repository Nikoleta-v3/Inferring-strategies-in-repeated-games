def best_responses(player_index, c, delta):
    if player_index in [0, 1, 3, 5, 7, 13, 15, 16, 17, 19]:
        return best_responses_to_d0_d1_d3_d4_d5_d7_d13_d15_d16_d17_d19(c, delta)
    if player_index == 2:
    ...
    if player_index in [24, 27]:
        return best_responses_to_d24_d27(c, delta)
    if player_index == 25:
        return best_responses_to_d25(c, delta)
    if player_index in [26]:
        return best_responses_to_d26(c, delta)


def best_responses_to_d0_d1_d3_d4_d5_d7_d13_d15_d16_d17_d19(c, delta):
    return [0, 4, 8, 12]
...
def best_responses_to_d24_d27(c, delta):
    list_of_brs1, list_of_brs2 = [], []
    if c >= delta:
        list_of_brs1 = [0, 4, 8, 12]
    if c <= delta:
